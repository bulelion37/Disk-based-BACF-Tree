...
public class KMEANS_TEST {
	...
	public static void main(String[] args) throws Exception {
		...
		String[] st = datasetFile.split("\\\\");
		String temp = st[13];
		st = temp.split("\\_");
		CFTree.outputFileName += st[0];
		CFTree.outputFileName += "_leaf_.txt";
		
		int lineNum = 500000;
		double[][] x = new double[lineNum][2];
		...
		while((line=in.readLine())!=null) {
			...
			//x[current] = new double[tmp.length];
			
			for(int i=0; i<x[current].length; i++) {
				x[current][i] = Double.parseDouble(tmp[i]);
			}
			current++;
		}
		
		CFTree birchTree = null;
		while(true)
		{
			birchTree = new CFTree(maxNodeEntries,distThreshold,CFTree.D0_DIST,applyMergingRefinement);
			birchTree.setMemoryLimit(1*256*512);
			birchTree.rebuildFlag = false;
			
			CFTree.CFTreeSize = SizeOf.sizeOf(birchTree);
			for(int i = 0 ; i < lineNum ; i++)
			{
				boolean inserted = birchTree.insertEntry(x[i]);
				if(!inserted) {
					System.err.println("NOT INSERTED!");
					System.exit(1);
				}
				CFTree.instanceIndex++;
				
				if(birchTree.rebuildFlag)
				{
					distThreshold = birchTree.root.getDistThreshold();
					break;
				}
				if(i % 1000 == 0)
