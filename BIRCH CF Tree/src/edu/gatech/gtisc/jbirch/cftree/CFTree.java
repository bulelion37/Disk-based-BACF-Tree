...
import edu.gatech.gtisc.jbirch.test.birchTree;
...
public class CFTree {
	...
	public static long CFTreeSize = 0;
	public static long CFNodeSize = 0;
	public static long CFEntrySize = 0;
	
	public boolean rebuildFlag = false;
	 ...
	 */
	private long memLimit = (long)Math.pow(1024, 3) * 3; // default = 1GB
	//private long memLimit = 1*512*1024;
	...
	private boolean hasReachedMemoryLimit(CFTree tree, long limit) {
		...
		long memory = computeMemorySizeByTheory(tree);
	...
	/**
	 * Computes the memory usage of a CFTree 
	 * computerMemorySize's some of size functions cause error 
	 * computes the memory usage of a CFTree by theory
	 * 
	 * @param t a CFTree
	 * @return memory usage in bytes
	 */
	public long computeMemorySizeByTheory(CFTree t) {
		
		long memSize = 0;
		long nodeNum = 0;
		long entryNum = 0;
		
		Queue<CFNode> q = new LinkedList<>();
		q.add(t.root);
		...
		memSize = CFTreeSize + CFNodeSize * nodeNum + CFEntrySize * entryNum;
	...
	/**
	 * 
	 * write leaf node information in txt file
	 * @throws IOException 
	 */
	public void writeLeafNodeInfo() throws IOException {
		
		System.out.println("Final Birch CF Tree Threshold = " + root.getDistThreshold());
		
		
		BufferedOutputStream bs = null;
		
		bs = new BufferedOutputStream(new FileOutputStream(outputFileName));
		...
		while(l!=null) {
			if(!l.isDummy()) {
				...
				for(CFEntry e : l.getEntries()){
					String one_line = ""; 
					one_line += Double.toString(e.sumX[0]/e.n);
					one_line += " ";
					one_line += Double.toString(e.sumX[1]/e.n);
					one_line += "|";
					one_line += Arrays.toString(e.getIndexList().toArray(new Integer[0]));
					one_line += "\n";
					bs.write(one_line.getBytes());
					lineNum++;
				}
			}
			l = l.getNextLeaf();
